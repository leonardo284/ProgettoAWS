<script setup>
  ...
  onMounted(async () => {
    try {
      ...
      // Estraggo le ultime 5 partite (filtrate per stato "FINITA" e ordinate per data decrescente)
      lastMatches.value = matchesData.filter(m => m.stato === 'FINITA')
                                      .sort((a, b) => new Date(b.dataOra) - new Date(a.dataOra))
                                      .slice(0, 5);

      // Elaborazione classifica completa
      const fullStandings = standingsData.map((row, index) => { 
        const teamInfo = teamsData.find(t => t.teamId === row.teamId);
        // restituisco la riga di classifica arricchita con logo e nome squadra
        return {
          ...row,
      ...
      // Calcolo classifica ristretta usandol'indice della riga del team corrente
      ...
      if (currentIndex !== -1) {
        ...
        // restringo la classifica a 7 righe centrata sulla squadra
  ...
  // funzione per navigare alla pagina di dettaglio della partita
  const goToMatchDetail = (matchId) => { router.push(`/match/${matchId}`); };
</script>
    ...
    <div v-else class="content">
      ...
      <section class="section">
        ...
        <div class="standings-mini">
          <StandingsTable :standings="filteredStandings" :highlightTeamId="team.teamId" />
        </div>
      </section>
    </div>
  </div>
